/*@
Class for managing key expansion by calculating the next round key
based on the previous key and key round
 */
...
class KeyManager {
    ...
    static int[][] getNextKey(int[][] prevKey, int keyRound){
        ...
        int[] rConstants = new int[]{roundConstants[keyRound], 0, 0, 0};

        int[] col0 = AESutils.getColumn(prevKey, 0);
        int[] col3 = RotWord(AESutils.getColumn(prevKey, 3));
        int[] w4 = SubBytes(col3, AES.sBox);
        // bitwise xOr
        for(int i=0; i<4; i++){
            nextKey[i][0] = w4[i] ^ rConstants[i] ^ col0[i];
        }
        for(int r=0; r<4; r++){
            for(int c=1; c<4; c++){
